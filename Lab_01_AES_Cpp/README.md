...
&emsp;&emsp;根据 FIPS-197 中的 AES 算法描述，实现 AES 加解密算法和密钥扩展算法。包括 AES-128、AES-192、AES-256 三种模式。

&emsp;&emsp;实验中，使用 C++ 语言实现了 AES 加解密算法，并进行了一系列优化以提高运算速度，包括将列混合矩阵的实现改为查表法，对轮密钥加、字节代换、行移位、列混合进行循环展开等。
...
&emsp;&emsp;说明：代码中通过类模板实现了 AES-128、AES-192、AES-256 三种不同的密钥长度，针对不同的密钥长度分别实现对应的密钥扩展函数。
...
&emsp;&emsp;由于代码中使用了 C++17 的特性，因此需要使用 C++17 编译器进行编译，编译命令如下：

```bash
g++ -std=c++17 main.cpp -o main.exe
```

&emsp;&emsp;编译完成后，运行程序，得到如下结果：
...
&emsp;&emsp;可以看到，AES128、AES192、AES256 加密解密结果均正确。
